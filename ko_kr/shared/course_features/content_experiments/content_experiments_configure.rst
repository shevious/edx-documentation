...
#. 그룹 설정 이름을 입력한다. 의미 있는 명칭을 이용 하는 것은 콘텐츠 실험을 만들 때 그룹 설정을 선택하기 때문이다. 학습자는 이 그룹 설정 이름을 알지 못한다.
...
.. important:: 그룹 설정의 명칭은 언제든지 변경할 수 있다. 그러나 운영중인 강좌에 현재 이용되는 그룹 설정의 성격을 변경하기 전에  `그룹 설정 수정을 위한 가이드라인`_  을 살펴본다.
...
#. 필요하다면 설정에서 그룹을 수정한다.  `실험 그룹 설정 생성하기`_  에 자세하게 안내되어 있다.
